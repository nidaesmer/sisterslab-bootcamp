...
const waterPokemons = pokemons.filter((pokemon) => pokemon.type === 'Water').map(pokemon => pokemon.name)
...
const doubledExperience = pokemons.map((pokemon)=>({name: `${pokemon.name}`, experience: `${pokemon.experience *2}`})
)
